...
export function parseJsonField<T>(raw: FormDataEntryValue | null, field: string, fallback: T): T {
...
export function parseBooleanField(raw: FormDataEntryValue | null, fallback: boolean): boolean {
  if (raw == null) return fallback;
  if (typeof raw !== "string") return fallback;
  return ["1", "true", "yes", "on"].includes(raw.toLowerCase());
}

export function parseNumberField(raw: FormDataEntryValue | null, field: string): number | null {
...
export function validateFocalPoint(value: unknown): FocalPoint | null {
  ...
  if (!Number.isFinite(x) || !Number.isFinite(y) || x < 0 || x > 1 || y < 0 || y > 1) {
    throw new Error("focalPoint.x and focalPoint.y must be numbers between 0 and 1");
  }
  return { x, y };
}
...
export function validateThumbnailRequests(value: unknown): ThumbnailRequest[] {
  ...
  return value.map((entry, index) => {
    ...
    if (!Number.isInteger(width) || width <= 0) throw new Error(`thumbnails[${index}].width must be a positive integer`);
    if (!Number.isInteger(height) || height <= 0) throw new Error(`thumbnails[${index}].height must be a positive integer`);
    if (!["jpeg", "png", "webp", "avif"].includes(String(format))) {
      throw new Error(`thumbnails[${index}].format must be jpeg, png, webp, or avif`);
    }
...
export function parseJsonQuery(requestUrl: URL, key: string): Record<string, unknown> | null {
...
export function buildSearchText(search: string | null, metadata: Record<string, unknown>, fallbackName: string): string {
  const fragments = [search ?? "", fallbackName];
  for (const [key, value] of Object.entries(metadata)) {
    fragments.push(key, typeof value === "string" ? value : JSON.stringify(value));
  }

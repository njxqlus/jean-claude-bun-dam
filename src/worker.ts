  ...
  private async execute(job: JobRecord) {
    try {
      if (job.type === "thumbnail.generate") {
        const assetId = await this.service.processThumbnailJob(job.payload as ThumbnailJobPayload);
        await this.service.db.completeJob(job.id);
        const pending = await this.service.db.countPendingThumbnailJobs(assetId);
        if (pending === 0) {
      ...
      if (job.type === "thumbnail.generate") {
        ...
        await this.service.db.updateAssetStatus(payload.assetId, "failed", message);
      }
      const runAfter = new Date(Date.now() + Math.min(job.attempts, 10) * 5000).toISOString();

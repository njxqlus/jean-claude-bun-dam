...
export class AssetService {
  ...
  async createAssetFromRequest(request: Request) {
    ...
    if (fileValue.size > this.config.maxUploadBytes) {
      throw new Error(`file exceeds MAX_UPLOAD_BYTES=${this.config.maxUploadBytes}`);
    }

    const metadata = parseJsonField<Record<string, unknown>>(formData.get("metadata"), "metadata", {});
    const search = typeof formData.get("search") === "string" ? String(formData.get("search")) : null;
    const temporary = parseBooleanField(formData.get("temporary"), false);
    const ttlSeconds = parseNumberField(formData.get("ttlSeconds"), "ttlSeconds");
    const focalPoint = validateFocalPoint(parseJsonField(formData.get("focalPoint"), "focalPoint", null));
    const thumbnails = validateThumbnailRequests(parseJsonField(formData.get("thumbnails"), "thumbnails", []));
    ...
    const imageMetadata = kind === "image" ? await extractImageMetadata(fileValue) : null;
    const typedMetadata: Record<string, unknown> = imageMetadata
      ...
      : kind === "image" && focalPoint
        ? { focalPoint }
        : {};
    const searchText = buildSearchText(search, metadata, fileValue.name || normalizedName);
    const expiresAt = ttlSeconds != null && ttlSeconds > 0 ? new Date(Date.now() + ttlSeconds * 1000).toISOString() : null;
    ...
    if (thumbnails.length > 0) {
      if (kind !== "image") {
        await this.db.updateAssetStatus(asset.id, "failed", "Thumbnail requests are only supported for image assets");
      } else {
  ...
  async getDerivativeFileResponse(id: string, name: string) {
    ...
    return new Response(object.body, {
      headers: {
        ...
        "content-disposition": withContentDisposition(`${name}.${derivative.mime_type.split("/")[1] ?? "bin"}`),
      },
    });
  }
  ...
  async processThumbnailJob(payload: ThumbnailJobPayload) {
    ...
    const derivative = await createDerivativeImage(sourceBlob, payload.request, sourceMetadata, payload.focalPoint);
    const storageKey = join(
      this.config.s3Prefix,
      asset.id.slice(0, 2),
      asset.id,
    ...
    await this.storage.put(storageKey, new Blob([derivative.bytes], { type: derivative.mimeType }), derivative.mimeType);
